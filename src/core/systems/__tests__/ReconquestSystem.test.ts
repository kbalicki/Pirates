...
describe("heldPopulationCeiling", () => {
  it("leaves a royal colony drifting toward its own numbers", () => {
    const world = makeWorld({ ports: { [FORT]: makePort(FORT) } });
    expect(heldPopulationCeiling(world, FORT)).toBe(getPortBaseline(FORT).population);
  });

  it("cuts the people a town keeps when it flies the black flag", () => {
    const world = makeWorld();
    const baseline = getPortBaseline(FORT).population;
    expect(heldPopulationCeiling(world, FORT))
      .toBe(Math.round(baseline * HELD_POPULATION_SHARE));
  });
  ...
  it("is unchanged for a colony one crown took off another", () => {
    ...
    expect(heldPopulationCeiling(french, FORT)).toBe(getPortBaseline(FORT).population);
  });

  it("says nothing about wealth, which imports handle instead", () => {
    // Capping wealth here as well was double counting, and the first attempt at
    // it settled a captured Port Royale at a wealth of 5. See the constant.
    expect(typeof heldPopulationCeiling(makeWorld(), FORT)).toBe("number");
  });
});
