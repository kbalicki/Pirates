 ...
 *
 * A town still flying its own flag recovers toward the royal baseline: the
 * crown pays for the walls, the powder and the men. A town that changed hands
 * has none of that, and its people will only raise so much militia for whoever
 * is holding the fort this month. Without this the garrison lever would be
 * pointless — the town would defend itself back to full strength in a season
 * and there would be nothing for the player to decide.
 */
export function heldDefenseCeiling(world: WorldState, portKey: string): number {
  const baseline = getPortBaseline(portKey).defense;
  return portChangedHands(world, portKey) ? Math.round(baseline * HELD_DEFENSE_SHARE) : baseline;
}
...
/** The relief expedition already at sea for this town, if there is one. */
export function activeExpeditionFor(world: WorldState, portKey: string): WorldEventState | undefined {
  return world.worldEvents.find(ev => ev.type === "reconquest" && ev.ports[0] === portKey);
}

/** Every relief expedition currently at sea. */
export function expeditionsInFlight(world: WorldState): WorldEventState[] {
  return world.worldEvents.filter(ev => ev.type === "reconquest");
}
...
export function resolveRelief(
  ...
  const playerPresent = playerPresentAt(world, portKey);
  ...
  if (port) {
    ...
    const updated: PortRuntimeState = held
      ? {
          ...port,
          garrison: stationed - garrisonLost,
  ...
  w = { ...w, worldEvents: w.worldEvents.filter(ev => ev.id !== event.id) };
  ...
  if (held) {
    gold = playerPresent ? Math.round(expedition.soldiers * WRECK_GOLD_PER_SOLDIER) : 0;
    w = {
      ...w,
      player: {
        ...w.player,
        gold: w.player.gold + gold,
        notoriety: w.player.notoriety + (playerPresent ? 8 : 4),
      },
    };
    w = addLogEntry(w, playerPresent ? "reconquest.log_held_present" : "reconquest.log_held", vars);
  ...
  if (playerPresent) {
    ...
    const partyLost = Math.round(landingParty(initial) * townLossFrac * 0.5);
    if (partyLost > 0) {
      w = writeBackForce(w, initial, { ...initial, crew: Math.max(0, initial.crew - partyLost) });
    }
  }

  return {
    result: { world: w, events, portKey, townLost: !held, playerPresent, defence, attack, garrisonLost, gold },
    rng: roll.state,
  };
}
...
export function tickReconquest(world: WorldState): ReconquestTick {
  ...
  return { world: { ...w, rng }, events, ownersChanged };
}
