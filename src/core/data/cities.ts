/**
 * City definitions for the Caribbean map.
 * Based on Sid Meier's Pirates! (2004 remake) — 45 cities, era 1680.
 *
 * Coordinate conversion from real-world to game map:
 *   x = (98 - lon) / 40 * 3200   (lon in degrees West)
 *   y = (30 - lat) / 23 * 2400   (lat in degrees North)
 *
 * Map bounds: 3200x2400 pixels covering 98°W–58°W, 30°N–7°N
 *
 * NOTE: Per-era city faction ownership configs are planned for
 * a future update. This file contains the default (1680 era) configuration.
 */

import { portId, factionId } from "../model/ids.ts";
import type { PortId, FactionId } from "../model/ids.ts";
import type { Vec2 } from "../model/WorldState.ts";

export type CitySize = "small" | "medium" | "large" | "capital";
export type WealthLevel = "poor" | "modest" | "prosperous" | "wealthy";

export type CityDef = {
  id: PortId;
  name: string;
  factionId: FactionId;
  type: "city" | "fort" | "outpost";
  pos: Vec2;
  dockRadius: number;
  marketLevel: number;
  shipyardLevel: number;
  produces: string[];
  demands: string[];
  population: CitySize;
  wealth: WealthLevel;
};

/** Convert real-world lon/lat to game map pixel coordinates. */
export function geoToMap(lon: number, lat: number): Vec2 {
  return {
    x: Math.round((98 - lon) / 40 * 3200),
    y: Math.round((30 - lat) / 23 * 2400),
  };
}

export const CITIES: Record<string, CityDef> = {
  // ===================================================================
  // SPANISH (22)
  // ===================================================================
  havana: {
    id: portId("havana"),
    name: "Havana",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(82.4, 23.1),
    dockRadius: 44,
    marketLevel: 5,
    shipyardLevel: 4,
    produces: ["sugar_cane", "tobacco", "rum"],
    demands: ["food", "water", "cocoa"],
    population: "large",
    wealth: "wealthy",
  },
  santiago: {
    id: portId("santiago"),
    name: "Santiago",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(75.8, 20.0),
    dockRadius: 40,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["sugar_cane", "rum"],
    demands: ["food", "water", "tobacco"],
    population: "medium",
    wealth: "prosperous",
  },
  santo_domingo: {
    id: portId("santo_domingo"),
    name: "Santo Domingo",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(69.9, 18.5),
    dockRadius: 44,
    marketLevel: 4,
    shipyardLevel: 3,
    produces: ["sugar_cane", "cocoa"],
    demands: ["food", "water", "rum"],
    population: "large",
    wealth: "wealthy",
  },
  san_juan: {
    id: portId("san_juan"),
    name: "San Juan",
    factionId: factionId("spain"),
    type: "fort",
    pos: geoToMap(66.1, 18.4),
    dockRadius: 40,
    marketLevel: 4,
    shipyardLevel: 3,
    produces: ["sugar_cane", "rum"],
    demands: ["food", "water", "cocoa"],
    population: "large",
    wealth: "wealthy",
  },
  cartagena: {
    id: portId("cartagena"),
    name: "Cartagena",
    factionId: factionId("spain"),
    type: "fort",
    pos: geoToMap(75.5, 10.4),
    dockRadius: 44,
    marketLevel: 5,
    shipyardLevel: 4,
    produces: ["cocoa", "sugar_cane"],
    demands: ["food", "water", "rum"],
    population: "large",
    wealth: "wealthy",
  },
  porto_bello: {
    id: portId("porto_bello"),
    name: "Puerto Bello",
    factionId: factionId("spain"),
    type: "fort",
    pos: geoToMap(79.7, 9.6),
    dockRadius: 40,
    marketLevel: 4,
    shipyardLevel: 3,
    produces: ["cocoa"],
    demands: ["food", "water", "rum", "tobacco"],
    population: "medium",
    wealth: "wealthy",
  },
  panama: {
    id: portId("panama"),
    name: "Panama",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(79.5, 8.95),
    dockRadius: 44,
    marketLevel: 5,
    shipyardLevel: 3,
    produces: ["cocoa", "sugar_cane"],
    demands: ["food", "water", "rum", "tobacco"],
    population: "large",
    wealth: "wealthy",
  },
  vera_cruz: {
    id: portId("vera_cruz"),
    name: "Vera Cruz",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(96.1, 19.2),
    dockRadius: 44,
    marketLevel: 4,
    shipyardLevel: 3,
    produces: ["sugar_cane", "tobacco"],
    demands: ["food", "water", "cocoa", "rum"],
    population: "large",
    wealth: "wealthy",
  },
  campeche: {
    id: portId("campeche"),
    name: "Campeche",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(90.5, 19.8),
    dockRadius: 40,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["tobacco", "sugar_cane"],
    demands: ["food", "water", "rum"],
    population: "medium",
    wealth: "modest",
  },
  maracaibo: {
    id: portId("maracaibo"),
    name: "Maracaibo",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(71.6, 10.7),
    dockRadius: 40,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["cocoa", "tobacco"],
    demands: ["food", "water", "rum"],
    population: "medium",
    wealth: "modest",
  },
  cumana: {
    id: portId("cumana"),
    name: "Cumaná",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(64.2, 10.5),
    dockRadius: 36,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["cocoa"],
    demands: ["food", "water", "rum", "tobacco"],
    population: "medium",
    wealth: "modest",
  },
  trinidad: {
    id: portId("trinidad"),
    name: "Trinidad",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(61.5, 10.5),
    dockRadius: 36,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["sugar_cane", "cocoa"],
    demands: ["food", "water", "rum"],
    population: "small",
    wealth: "modest",
  },
  gran_granada: {
    id: portId("gran_granada"),
    name: "Gran Granada",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(85.0, 12.1),
    dockRadius: 40,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["sugar_cane", "cocoa"],
    demands: ["food", "water", "rum"],
    population: "medium",
    wealth: "wealthy",
  },
  caracas: {
    id: portId("caracas"),
    name: "Caracas",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(67.0, 10.5),
    dockRadius: 40,
    marketLevel: 4,
    shipyardLevel: 2,
    produces: ["cocoa", "tobacco"],
    demands: ["food", "water", "rum"],
    population: "medium",
    wealth: "wealthy",
  },
  gibraltar: {
    id: portId("gibraltar"),
    name: "Gibraltar",
    factionId: factionId("spain"),
    type: "outpost",
    pos: geoToMap(80.0, 22.0),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["sugar_cane"],
    demands: ["food", "water", "rum"],
    population: "small",
    wealth: "modest",
  },
  margarita: {
    id: portId("margarita"),
    name: "Margarita",
    factionId: factionId("spain"),
    type: "outpost",
    pos: geoToMap(63.9, 11.0),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["cocoa", "tobacco"],
    demands: ["food", "water", "rum"],
    population: "small",
    wealth: "prosperous",
  },
  nombre_de_dios: {
    id: portId("nombre_de_dios"),
    name: "Nombre De Dios",
    factionId: factionId("spain"),
    type: "outpost",
    pos: geoToMap(79.6, 9.5),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["cocoa"],
    demands: ["food", "water"],
    population: "small",
    wealth: "poor",
  },
  puerto_cabello: {
    id: portId("puerto_cabello"),
    name: "Puerto Cabello",
    factionId: factionId("spain"),
    type: "outpost",
    pos: geoToMap(68.0, 10.5),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["cocoa"],
    demands: ["food", "water", "rum"],
    population: "small",
    wealth: "modest",
  },
  puerto_principe: {
    id: portId("puerto_principe"),
    name: "Puerto Príncipe",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(77.9, 21.4),
    dockRadius: 40,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["sugar_cane", "tobacco"],
    demands: ["food", "water", "rum"],
    population: "medium",
    wealth: "modest",
  },
  rio_de_la_hacha: {
    id: portId("rio_de_la_hacha"),
    name: "Río de la Hacha",
    factionId: factionId("spain"),
    type: "outpost",
    pos: geoToMap(72.9, 11.5),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["tobacco"],
    demands: ["food", "water"],
    population: "small",
    wealth: "poor",
  },
  santa_catalina: {
    id: portId("santa_catalina"),
    name: "Santa Catalina",
    factionId: factionId("spain"),
    type: "outpost",
    pos: geoToMap(81.4, 13.4),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["sugar_cane"],
    demands: ["food", "water"],
    population: "small",
    wealth: "modest",
  },
  santa_marta: {
    id: portId("santa_marta"),
    name: "Santa Marta",
    factionId: factionId("spain"),
    type: "outpost",
    pos: geoToMap(74.2, 11.2),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["tobacco"],
    demands: ["food", "water"],
    population: "small",
    wealth: "poor",
  },
  st_augustine: {
    id: portId("st_augustine"),
    name: "St. Augustine",
    factionId: factionId("spain"),
    type: "fort",
    pos: geoToMap(81.3, 29.9),
    dockRadius: 36,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["food"],
    demands: ["water", "rum", "tobacco"],
    population: "small",
    wealth: "modest",
  },
  villa_hermosa: {
    id: portId("villa_hermosa"),
    name: "Villa Hermosa",
    factionId: factionId("spain"),
    type: "city",
    pos: geoToMap(92.9, 18.0),
    dockRadius: 40,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["tobacco", "sugar_cane"],
    demands: ["food", "water", "rum"],
    population: "medium",
    wealth: "modest",
  },

  // ===================================================================
  // ENGLISH (10)
  // ===================================================================
  port_royal: {
    id: portId("port_royal"),
    name: "Port Royale",
    factionId: factionId("england"),
    type: "city",
    pos: geoToMap(76.8, 17.9),
    dockRadius: 40,
    marketLevel: 4,
    shipyardLevel: 4,
    produces: ["rum", "food"],
    demands: ["sugar_cane", "cocoa", "tobacco"],
    population: "medium",
    wealth: "prosperous",
  },
  nassau: {
    id: portId("nassau"),
    name: "Nassau",
    factionId: factionId("england"),
    type: "city",
    pos: geoToMap(77.3, 25.0),
    dockRadius: 36,
    marketLevel: 2,
    shipyardLevel: 2,
    produces: ["rum"],
    demands: ["food", "water"],
    population: "small",
    wealth: "modest",
  },
  barbados: {
    id: portId("barbados"),
    name: "Barbados",
    factionId: factionId("england"),
    type: "city",
    pos: geoToMap(59.6, 13.1),
    dockRadius: 44,
    marketLevel: 4,
    shipyardLevel: 3,
    produces: ["sugar_cane", "rum"],
    demands: ["food", "water", "tobacco"],
    population: "large",
    wealth: "wealthy",
  },
  antigua: {
    id: portId("antigua"),
    name: "Antigua",
    factionId: factionId("england"),
    type: "city",
    pos: geoToMap(61.8, 17.1),
    dockRadius: 36,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["sugar_cane"],
    demands: ["food", "water", "rum"],
    population: "small",
    wealth: "modest",
  },
  st_kitts: {
    id: portId("st_kitts"),
    name: "St. Kitts",
    factionId: factionId("england"),
    type: "city",
    pos: geoToMap(62.7, 17.3),
    dockRadius: 36,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["sugar_cane"],
    demands: ["food", "water", "rum", "tobacco"],
    population: "small",
    wealth: "modest",
  },
  belize: {
    id: portId("belize"),
    name: "Belize",
    factionId: factionId("england"),
    type: "outpost",
    pos: geoToMap(88.2, 17.5),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["rum"],
    demands: ["food", "water", "tobacco"],
    population: "small",
    wealth: "modest",
  },
  bermuda: {
    id: portId("bermuda"),
    name: "Bermuda",
    factionId: factionId("england"),
    type: "outpost",
    pos: geoToMap(64.8, 29.5),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["food"],
    demands: ["water", "rum", "tobacco"],
    population: "small",
    wealth: "modest",
  },
  eleuthera: {
    id: portId("eleuthera"),
    name: "Eleuthera",
    factionId: factionId("england"),
    type: "outpost",
    pos: geoToMap(76.2, 25.0),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["rum"],
    demands: ["food", "water"],
    population: "small",
    wealth: "poor",
  },
  gran_bahama: {
    id: portId("gran_bahama"),
    name: "Gran Bahama",
    factionId: factionId("england"),
    type: "outpost",
    pos: geoToMap(78.5, 26.6),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["rum"],
    demands: ["food", "water"],
    population: "small",
    wealth: "poor",
  },
  nevis: {
    id: portId("nevis"),
    name: "Nevis",
    factionId: factionId("england"),
    type: "outpost",
    pos: geoToMap(62.6, 17.1),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["sugar_cane"],
    demands: ["food", "water", "rum"],
    population: "small",
    wealth: "modest",
  },

  // ===================================================================
  // FRENCH (8)
  // ===================================================================
  tortuga: {
    id: portId("tortuga"),
    name: "Tortuga",
    factionId: factionId("france"),
    type: "outpost",
    pos: geoToMap(72.8, 20.1),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["rum"],
    demands: ["food", "water"],
    population: "small",
    wealth: "modest",
  },
  martinique: {
    id: portId("martinique"),
    name: "Martinique",
    factionId: factionId("france"),
    type: "city",
    pos: geoToMap(61.0, 14.6),
    dockRadius: 40,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["cocoa", "sugar_cane", "rum"],
    demands: ["food", "water", "tobacco"],
    population: "medium",
    wealth: "prosperous",
  },
  guadeloupe: {
    id: portId("guadeloupe"),
    name: "Guadeloupe",
    factionId: factionId("france"),
    type: "city",
    pos: geoToMap(61.5, 16.2),
    dockRadius: 36,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["sugar_cane", "cocoa"],
    demands: ["food", "water", "rum"],
    population: "medium",
    wealth: "prosperous",
  },
  petit_goave: {
    id: portId("petit_goave"),
    name: "Petit Goâve",
    factionId: factionId("france"),
    type: "outpost",
    pos: geoToMap(72.9, 18.4),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["sugar_cane", "rum"],
    demands: ["food", "water", "cocoa"],
    population: "small",
    wealth: "modest",
  },
  port_de_paix: {
    id: portId("port_de_paix"),
    name: "Port de Paix",
    factionId: factionId("france"),
    type: "outpost",
    pos: geoToMap(72.8, 19.9),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["rum"],
    demands: ["food", "water", "sugar_cane"],
    population: "small",
    wealth: "modest",
  },
  florida_keys: {
    id: portId("florida_keys"),
    name: "Florida Keys",
    factionId: factionId("france"),
    type: "outpost",
    pos: geoToMap(81.8, 24.7),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["food"],
    demands: ["water", "rum"],
    population: "small",
    wealth: "poor",
  },
  leogane: {
    id: portId("leogane"),
    name: "Léogane",
    factionId: factionId("france"),
    type: "outpost",
    pos: geoToMap(72.6, 18.5),
    dockRadius: 32,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["sugar_cane", "rum"],
    demands: ["food", "water"],
    population: "small",
    wealth: "modest",
  },
  montserrat: {
    id: portId("montserrat"),
    name: "Montserrat",
    factionId: factionId("france"),
    type: "outpost",
    pos: geoToMap(62.2, 16.7),
    dockRadius: 32,
    marketLevel: 1,
    shipyardLevel: 1,
    produces: ["sugar_cane"],
    demands: ["food", "water"],
    population: "small",
    wealth: "modest",
  },

  // ===================================================================
  // DUTCH (3)
  // ===================================================================
  curacao: {
    id: portId("curacao"),
    name: "Curaçao",
    factionId: factionId("netherlands"),
    type: "city",
    pos: geoToMap(69.0, 12.2),
    dockRadius: 36,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["tobacco", "cocoa"],
    demands: ["food", "water", "sugar_cane"],
    population: "small",
    wealth: "prosperous",
  },
  st_eustatius: {
    id: portId("st_eustatius"),
    name: "St. Eustatius",
    factionId: factionId("netherlands"),
    type: "city",
    pos: geoToMap(62.9, 17.5),
    dockRadius: 36,
    marketLevel: 3,
    shipyardLevel: 2,
    produces: ["tobacco"],
    demands: ["food", "water", "sugar_cane", "rum"],
    population: "small",
    wealth: "prosperous",
  },
  st_martin: {
    id: portId("st_martin"),
    name: "St. Martin",
    factionId: factionId("netherlands"),
    type: "city",
    pos: geoToMap(63.1, 18.1),
    dockRadius: 36,
    marketLevel: 2,
    shipyardLevel: 1,
    produces: ["tobacco"],
    demands: ["food", "water", "rum"],
    population: "small",
    wealth: "modest",
  },
};
