...
/** Seagull cry from MP3 file — plays at zoom 9+ only */
const SEAGULL_CRY_INTERVAL_MIN = 12_000;  // ms between cries
const SEAGULL_CRY_INTERVAL_MAX = 30_000;
const SEAGULL_CRY_ZOOM_MIN = 9;  // only play at zoom 9+
...
export class SeagullRenderer {
  ...
  private seagullSound: Phaser.Sound.BaseSound | null = null;
  ...
  update(windDirRad: number, windStrength: number): void {
    ...
    // Seagull cry sounds at zoom 9+ — only if seagulls visible on screen
    if (cam.zoom >= SEAGULL_CRY_ZOOM_MIN && this.seagulls.some(g => g.gameObject.visible)) {
      const now = Date.now();
      if (now >= this.nextCryTime) {
        try {
          if (!this.seagullSound && this.scene.cache.audio.exists("seagull")) {
