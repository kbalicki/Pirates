...
**Stan na:** 2026-09-17 · **Wersja:** v0.69.0.0 · **Branch:** `main`
**Kod:** 241 plików `.ts` · `tsc --noEmit` czysty · `npm test` — **2086 przechodzi, 0 failuje, 0 `todo`** w 64 plikach
...
> **Notatka z tej sesji:** [SESSION-2026-09-17c.md](documentation/SESSION-2026-09-17c.md) (v0.69.0 — polskie przypadki nazw)
...
**Zmierzone w tej sesji, nienaprawione — osiem starych opowieści z tawerny.**
Zanim wziąłem przypadki, przemiotłem `RumorSystem`. Od v0.28.0 tawerna **relacjonuje
świat**, ale ośmioelementowa lista `FLAVOUR_KEYS` z czasów sprzed tamtego wydania wciąż
jest w puli i dochodzi do głosu, **kiedy świat nie ma nic do powiedzenia**. Zmierzone
na świeżym świecie (`rumorsAt` dla wszystkich 45 portów, dzień 1): **45/45 portów jest
ciche**, czyli te osiem zdań to **wszystko, co gracz może usłyszeć przy pierwszej wizycie
w tawernie**. A trzy z nich są nieprawdą, którą da się sprawdzić:

1. **„Ceny cukru w Barbados biją rekordy”** — Barbados **uprawia cukier**
   (`produces: ["sugar_cane", "rum"]`). Zmierzone na osiadłej gospodarce: Barbados jest
   **34. z 45** pod względem ceny cukru (4 zł przy 27 w Port Royale). Kapitan, który w to
   uwierzy, wiezie ładunek tam, gdzie zapłacą mu najmniej w całym Karaibie
2. **„Ludzie Czarnobrodego napadają na statki koło Nassau”** — Czarnobrody **nie istnieje
   w grze** (jedyne wystąpienie w całym `src` to ta linijka w pliku locale), a jego kariera
   (1716–1718) zaczyna się **trzydzieści sześć lat po najpóźniejszej epoce** (`pirates_sunset`,
   1680). Kandydat na zamianę: własna sława gracza odbita z powrotem (`world.playerName`
   + próg `notoriety`) — to jest ta sama sztuczka, co u Sida Meiera
3. **„Hiszpańską flotę skarbową widziano w Cieśninie Nawietrznej”** — flota skarbowa jest
   od v0.46.0 **prawdziwym wydarzeniem**, z prawdziwym portem zbiórki i prawdziwą trasą,
   i ta trasa **nie idzie Cieśniną Nawietrzną**: zbiórka w Puerto Bello / Nombre de Dios /
   Vera Cruz, rendez-vous w Hawanie, wyjście **Cieśniną Florydzką** (własny komentarz
   `TreasureFleetSystem`). Gorsze jest to, czego **nie ma**: `rumorsAt` **w ogóle nie zna
   floty skarbowej** — najcenniejsza rzecz w grze nie przechodzi przez kanał zbudowany po to,
   żeby relacjonować świat, a zamiast niej leci zmyślona wersja z niewłaściwą wodą

Do tego: **„Gubernator szuka zdolnych kapitanów”** jest prawdą tylko wtedy, gdy
`offerFor()` coś zwraca (wymaga standingu `sojusznik` **i** wyprawy w drodze — na starcie
kampanii nigdy), a **„na południu zbiera się wielki sztorm”** mówi się w lutym tak samo jak
we wrześniu, chociaż huragan ma w kodzie **sezon** (`seasonal: [6..11]`) i chodzi **drogą
po nazwanych miastach**, nie „na południu”. Jedyne, co powinno zostać nietknięte, to statek
widmo koło Bermudów — bo nie twierdzi niczego o świecie.

Kształt naprawy jest już przemyślany: flavour przestaje być listą kluczy, a staje się
tabelą `{ key, when?, vars? }` — zdanie, które coś twierdzi o świecie, jest **oferowane
tylko wtedy, gdy świat się zgadza**; zdanie, którego świat nie umie sprawdzić, jest
przepisane tak, żeby niczego nie twierdziło. Flota skarbowa i huragan w zasięgu dochodzą
do `rumorsAt` jako **fakty**, nie jako ozdoba.
...
- **Produkcja producenta jest wielokrotnie większa niż odpływ** (zmierzone w v0.67.0,
  świadomie nieruszone). Szopa producenta stoi **na suficie zawsze** (0 z 86 szlaków
  poniżej połowy), więc `RESTOCK_SURGE` u niego **nigdy nie odpala**, nadwyżka jest co dnia
  wyrzucana, a zdarzenia ruszające `productionMul` — żniwa ×1,8, bunt ×0,3 — są u źródła
  **niewidoczne**: bunt niewolników w Hawanie nie zmienia ceny cukru przez sześćdziesiąt
  dni. Sufit **nie jest** na to lekarstwem (sprawdzone: ×10 odwraca szlaki) — lekarstwem
  byłoby zbliżenie produkcji do odpływu, a to jest przebudowa, nie strojenie
