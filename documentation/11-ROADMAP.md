...
- ✅ **Dzień pierwszy świata** (v0.57.0) — `seedInitialEvents` było **drugim czytaniem** tabeli `RANDOM_EVENTS`, rozjechanym z codziennym losowaniem na pięć sposobów. Najdroższy: strażnik `worldEvents.length > 0` przestał znaczyć „jest już zasiane”, gdy v0.31.0 postawiło przed nim `seedHistoricalWars` — **trzy z sześciu er (1600, 1620, 1640) otwierały się bez żadnych żywych zdarzeń**, przez dwadzieścia pięć wydań. Do tego huragan w styczniu (16,5% zasianów), szablony losowane równomiernie zamiast po `weight` (najazd piratów 0,47× udziału), dekret jednej korony na 24 kolonie **Hiszpanii** (44,7% dekretów) i huragan nad trzema przystańmi sprłaszczony do jednej (3300/3300). Oba wejścia idą teraz przez jedną funkcję `rollOneEvent`. Przy okazji tablica ogłoszeń przestała być stosem: sortowana po **zasięgu**, więc własny news miasta stoi nad dekretem korony
- ✅ **Przyjaciel przyjaciela** (v0.56.0) — sojusz z v0.55.0 nie decydował o niczym, czego gracz mógłby dotknąć. Teraz **papier podróżuje**: komisja jednej korony jest czytana w porcie korony bijącej się w tej samej wojnie (stopień wyżej przy ladzie, powód wypisany w nagłówku), a `alliedWith` dostaje trzecie wejście — **mury sojusznika patrona**, czyli płatne zlecenie obrony i rozgrywalna bitwa. 18,8% dni, 14,5 z 45 miast. Awans wyłącznie od `neutral` w górę: sojusz ministrów nie jest amnestią
- ✅ **Sojusz jest bytem świata** (v0.55.0) — współwalczenie z v0.51.0 przesuwało wyłącznie relację, a `areAllied` nie miało przez cztery wydania **ani jednego czytelnika**. Dwie korony mają wspólnego wroga przez **28,8% dni**, w epizodach po 16 miesięcy: stan zostaje wyliczany, ale **początek jest stemplowany** w zdarzeniu `alliance`, więc tablice newsów, NPC, tawerna i zakładka Kapitan mają wreszcie co powiedzieć. Na wodzie: **wspólna wyprawa** — kontyngent sojusznika w desancie i jego bandera na ostatnim kadłubie linii
- ✅ **Ona pracuje na wiatr, i widać, jak to robi** (v0.54.0) — pasmo halsu napełnia się w 12°, nie w 30, więc najlepszy hals stoi tam, gdzie szukałby go marynarz (42° / 67°); droga na wiatr w górę o połowę. Kompas rysuje klin martwej strefy i oba najlepsze halsy, HUD pokazuje drogę faktycznie robioną i znos prądu
- ✅ **Martwa strefa nie była martwa** (v0.53.0) — halsowanie było ściśle gorsze od płynięcia prosto pod wiatr, dla **wszystkich dziewięciu klas**, przy zwykłym pasacie. Dodatek na słaby wiatr wypłacał się w oku wiatru; teraz idzie proporcjonalnie do tego, ile płótna ciągnie. Ciężki żaglowiec rejowy jest wreszcie statkiem **z wiatrem**, co `minWindAngle` obiecywał od v0.9.4
- ✅ **Wróg mojego wroga** (v0.52.0) — co zrobisz jednej koronie, czytają wszystkie pozostałe przez własne zatargi. Reputacja przestała być czterema niezależnymi liczbami, a sojusz z v0.51.0 dostał drugą rolę: teraz **kosztuje**
- ✅ **Korony kłócą się same** (v0.51.0) — `DiplomacySystem`: domyślna era spędzała w wojnie 7% pierwszych dziesięciu lat, przez co **list kaperski nie krył nigdy niczego** i sześć systemów poniżej wojny nie wykonało się ani razu. Szanse biorą się z macierzy relacji czytanej dotąd przez jedną linijkę, a sojusz to wspólny wróg i nic więcej
- ✅ **Karaiby mają w sobie piratów** (v0.50.0) — `PredationSystem`: korsarze wreszcie pływają (fitują się z 18 outpostów pod czarną banderą), a `aggression` i `targetEntityId` — dwa pola czytane dotąd przez nic — niosą cudze pościgi. Jeden kadłub na trzy dni ginie w walce, która nie jest walką gracza
- ✅ **Statek potrzebuje rąk** (v0.49.0) — `CrewSystem`: `crewMin` z tabeli klas przestał być dekoracją. Obsada pryzu bierze się z własnego pokładu i z przymuszonych, a brak rąk zabiera sterowność wcześniej niż prędkość — slup obsadzi brygantynę, ale nie galeon
- ✅ **Woda ma dno** (v0.48.0) — `SeaDepth`: zanurzenie z tabeli klas kontra głębokość wyprowadzona z linii brzegowej. Małe kadłuby wszędzie, duże zamknięte przed 6,2% morza; porty pogłębione, bo **wszystkie 45** podejść leży w płyciźnie
- ✅ **Pozostałe trzy umiejętności** (v0.47.0) — `navigation` i `medicine` nie były czytane przez **nikogo**, a `gunnery` wyłącznie przez ostrzał murów. Teraz: `navigatedWindModifier` (nawigator wart tyle, ile wiatr przeciwny), `gunneryAccuracy` (bitwa morska) i `SurgeonSystem` (lazaret). Ten sam błąd co z `setSwordsmanship`, trzy razy — **umiejętność bez odbiorcy wygląda dokładnie jak umiejętność działająca**
...
- ✅ **Wioski Indian** (v0.58.0) — osiem lokacji nie-portowych, żadna pod koroną. `native_raid` był od v0.9.7 **skutkiem bez przyczyny**: kompletne zdarzenie, którego nikt nie mógł wywołać, przy ekranie pomocy obiecującym, że osłabiony fort to okazja dla pirata. Rum za złoto (jedyne dobro `rare`, którego żaden port nie trzyma), stosunek z dwiema połowami — wyprowadzaną z reputacji u sąsiedniej korony **czytanej na opak** i stemplowaną — a przy 60 wyprawa wojenna na sąsiednią kolonię. Cztery metry nad lądowiskiem wykluczają głębokie kadłuby: pierwszy nowy czytelnik `draft` od v0.48.0. Bez nowego pola obowiązkowego — migracje na v12
- **Misje jezuickie** — leczenie załogi, informacje o regionie, konwersja piratów. **Odłożone w v0.58.0 z pomiarem**: przy `medicine` 5 lazaret oddaje już 68% rannych, więc misja lecząca wszystkich jest warta około jednego człowieka na walkę — musi sprzedawać coś innego niż leczenie
...
- **Flota skarbowa** — ✅ v0.46.0. `treasure_fleet` był nagłówkiem od v0.9.7; teraz płyną cztery kadłuby (dwa galeony ze srebrem, dwa z samymi działami) trasą Puerto Bello/Vera Cruz → Hawana → Cieśnina Florydzka, którą wyznaczyła tabela prądów, a nie ręka projektanta. Kadłub wart ~8300, cena srebra spada z 77 do 31 za tonę, a to, co nie dotarło do Sewilli, zabiera Hiszpanii bogactwo w każdej kolonii
...
- ✅ **Głód nie jest hossą** (v0.64.0) — `priceMul` był **jedną liczbą na cały port**, a trzy zdarzenia o jedzeniu używały właśnie jej: zmierzone na 6 ziarnach × 10 lat, głód podnosił cenę **tytoniu** ×2,32 i złota ×2,32 — tyle samo, co żywności, o którą w nim chodzi, a dobre żniwa cukru taniły złoto o 30%. **Czterdzieści ton tytoniu w głodującym mieście dawało 3551 zł zamiast 1534** (+131%), czyli głód był najlepszą rzeczą, jaka mogła spotkać kapitana wiozącego cokolwiek — odwrotność tego, po co powstały zamówienie na dostawę i spichlerz. Mnożnik ma teraz **temat** (`itemPriceMul` + `priceMulFor`), a tarify, hossa i wybuch wojny zostają ogólnorynkowe. **Tym razem podręcznik miał rację, a kod nie**
- ✅ **Gra mówi jednym głosem** (v0.63.0) — tabele danych niosły **drugą, wyłącznie angielską kopię** nazw, które tabele locale już miały, i rdzeń budował z niej `vars` dla każdego nagłówka, wpisu do dziennika i celu questa: *„Dekret królewski **Spain** zmienia taryfy w koloniach”*. Ponieważ `vars` są **stemplowane i zapisywane**, stempel niesie teraz **klucz**, a `t()` rozwija go przy podstawianiu — jedno miejsce zmiany zamiast każdego renderera, bez migracji, a zmiana języka przepisuje także stary dziennik. **99 odczytów** tego pola w kodzie produkcyjnym, z czego pięć znalazło dopiero **skasowanie pola**. Przy okazji: trzecia kopia nazw koron, martwe `ShipClassDef.nameKey`, angielskie dni tygodnia w kalendarzu i **zły rok w zakładce Kalendarz w pięciu z sześciu er**
- ✅ **Wieści są czymś, po co się podchodzi** (v0.62.0) — nośnikiem wieści jest statek od v0.28.0, ale kanały były dwa i pierwszy chodził ten zły: cicha reguła na trzydziestu jednostkach zabierała **trzy** pozycje z tablicy noszącej średnio **2,36** — czyli całą — **zanim** ekran spotkania na osiemnastu mógł cokolwiek dać, a `npc_news` nie miał **żadnego `case`** w rendererze, więc na czarcie rosły znaki, o których nikt nie powiedział słowa. Teraz zawołanie jest warte **jedną** pozycję (tę z czoła tablicy, sortowanej po zasięgu od v0.57.0) i mówi o tym toastem, a resztę kupuje podejście — i jej odpowiedź umie wreszcie powiedzieć „nic, czego byś nie słyszał”
 — ekran pomocy obiecywał *„+50 bogactwa, możliwy reset reputacji”* od **v0.9.7.1**, pięćdziesiąt jeden wydań, a zdarzenie dopisywało miastu wyłącznie pięćdziesiąt złotych. Nie była to ozdoba: standing poniżej `neutral` zamyka list kaperski, zlecenie obrony, córkę gubernatora i kantor frachtowy, więc jedyną drogą w górę był spichlerz za **+8** w głodującym mieście. Pomiar (20 ziaren × 50 lat: **7975** nominacji, 6% wszystkich zdarzeń, ok. 8 rocznie, mediana odstępu u Hiszpanii **65 dni**) zabił dosłowne czytanie — amnestia całej korony co dwa miesiące to wyłącznik reputacji. Wydane jako `PardonSystem`: **lokalne**, kupowane w ciągu 30 dni od nominacji, podnosi **do `neutral` i nie wyżej**, obejmuje kartotekę z dnia podpisu i pada, gdy kapitan da nowy powód. Pole opcjonalne na mieście — migracje dalej v12
